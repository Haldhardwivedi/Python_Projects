from tkinter import *
import parser
import math

root = Tk()
#title

root.title("Calculator")

i=0;
#utility functions 

def get_variables(num):
    global i
    display.insert(i,num)
    i+=1

def get_operation(operator):
    global i 
    length=len(operator)
    display.insert(i,operator)
    i+=length


def clear_all():
    display.delete(0,END)

def undo():
    entire_string=display.get()
    if len(entire_string):
        new_str=entire_string[:-1]
        clear_all()
        display.insert(0,new_str)
    else :
        clear_all()
        

def calucations():
    entire_string= display.get()
    try :
        a=parser.expr(entire_string).compile()
        result=eval(a)
        clear_all()
        display.insert(0,result)
    except Exception :
        clear_all()
        display.insert(0,"Error")

def fact():
    entire_string =display.get()
    try :
        num=int(entire_string)
        clear_all()
        num=math.factorial(num)
        display.insert(0,num)
    except Exception:
        clear_all()
        display.insert(0,"Error")

#adding the input filed
display =Entry(root)
display.grid(row=0, columnspan=6, sticky=W+E)

def uf(root):
    root.columnconfigure(0, pad=3)
    root.columnconfigure(1, pad=3)
    root.columnconfigure(2, pad=3)
    root.columnconfigure(3, pad=3)
    root.columnconfigure(4, pad=3)
    root.columnconfigure(5, pad=3)

    root.rowconfigure(0, pad=3)
    root.rowconfigure(1, pad=3)
    root.rowconfigure(2, pad=3)
    root.rowconfigure(3, pad=3)
    root.rowconfigure(4, pad=3)
    root.rowconfigure(5, pad=3)
    root.rowconfigure(6, pad=3)

uf(root)
#adding buttons 

Button(root,text='1',command=lambda:get_variables(1)).grid(row=1,column=0)
Button(root,text='2',command=lambda:get_variables(2)).grid(row=1,column=1)
Button(root,text='3',command=lambda:get_variables(3)).grid(row=1,column=2)

Button(root,text='4',command=lambda:get_variables(4)).grid(row=2,column=0)
Button(root,text='5',command=lambda:get_variables(5)).grid(row=2,column=1)
Button(root,text='6',command=lambda:get_variables(6)).grid(row=2,column=2)

Button(root,text='7',command=lambda:get_variables(7)).grid(row=3,column=0)
Button(root,text='8',command=lambda:get_variables(8)).grid(row=3,column=1)
Button(root,text='9',command=lambda:get_variables(9)).grid(row=3,column=2)

Button(root,text='AC',command=lambda:clear_all()).grid(row=4,column=0)
Button(root,text='0',command=lambda:get_variables(0)).grid(row=4,column=1)
Button(root,text='.',command=lambda:get_variables('.')).grid(row=4,column=2)

Button(root,text='+',command=lambda:get_operation("+")).grid(row=1,column=3)
Button(root,text='-',command=lambda:get_operation("-")).grid(row=2,column=3)
Button(root,text='*',command=lambda:get_operation("*")).grid(row=3,column=3)
Button(root,text='/',command=lambda:get_operation("/")).grid(row=4,column=3)

Button(root,text="<-",command=lambda:undo()).grid(row=1,column=4)
Button(root,text="%",command=lambda:get_operation("%")).grid(row=2,column=4)
Button(root,text="^2",command=lambda:get_operation("**2")).grid(row=3,column=4)
Button(root,text="^3",command=lambda:get_operation("**3")).grid(row=4,column=4)

Button(root,text="=",command=lambda:calucations()).grid(row=1,column=5)
Button(root,text="x!",command=lambda:fact()).grid(row=2,column=5)
Button(root,text="(",command=lambda:get_operation("(")).grid(row=3,column=5)
Button(root,text=")",command=lambda:get_operation(")")).grid(row=4,column=5)


root.mainloop()

